...
describe("Answer Component", () => {
    ...
    beforeEach(() => {
        ...
        spyOn(pollService, "addPollVote").and.returnValue(Observable.of(poll));

        answerComponent = new AnswerComponent(pollService, router, null);
    });

    it("should call PollService.getPoll() during ngOnInit()", () => {
        answerComponent.ngOnInit();

        expect(pollService.getPoll).toHaveBeenCalled();
    });

    it("should call PollService.addPollVote() during onVoteButtonTapped()", () => {
        answerComponent.onVoteButtonTapped({
            "button": {
                "bindingContext": 1
            }
        });
